...
summary AS (
  SELECT
    ...
    (SELECT COUNT(*) FROM marker_owned_canonical_exact) AS canonical_found_count,
    ...
    (SELECT COUNT(*) FROM marker_owned_aliases_exact) AS alias_found_count,
...
SELECT
  ...
  CASE
    WHEN analysis_marker_exact_count = 1
     AND analysis_marker_conflict_count = 0
     AND canonical_found_count = expected_canonical_count

...
- **JTBD-4 "믿을 만해?"** → 성분 상세(효능/부작용/논문) → 리뷰 AI 요약.
...
```
**B. 검색→분석:** 탐색 → 자동완성/오타보정 → 결과 → 제품상세 → "AI 정밀 분석" → 결과.
...
- AI 분석: 3단계 progress("성분 읽는 중 → 위험도 계산 → 우리 애와 매칭") — 대기시간을 스토리로.
...
- **AI 리뷰 요약:** 리뷰 다수 → "장점 3 / 단점 2" 요약 + 원문 링크.
...
## 34. 2026 UX Trend 반영
- **AI 우선 진입**: 홈 최상단 "스캔 = 즉답". 대기시간을 설명형 progress로.
- **Spatial/Soft depth**: 유리·소프트 그림자·라운드(현행 톤 유지, 과한 글래스는 성능 고려 절제).
- **Adaptive personalization**: 프로필·시간대 기반 홈 재구성.
- **Haptic-rich micro-feedback**, **정직한 AI(불확실성 표기)**, **웰빙형 게이미피케이션**(강박 아닌 습관).
...
|---|---|---|---|---|
| 1 | Header | back, "AI 프리미엄 영양 리포트" | Title-1, line bottom | back aria "뒤로" |

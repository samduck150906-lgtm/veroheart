...
현재 자동 preflight에서 BLOCK이 없을 때 가능한 최선 결과는 `REVIEW_REQUIRED`입니다. 실제 migration history의 version row를 의도적으로 읽지 않아 `migration_version_20260630090000` 검사가 항상 WARN이기 때문입니다. 따라서 현재 SQL에서 `PREFLIGHT_PASS`가 나올 것으로 기대하면 안 됩니다.
...
이 분기는 향후 migration-history 확인이 자동 점검에 통합될 경우를 위한 예약 상태입니다. 현재 SQL 단독 실행에서는 도달할 수 없습니다. 향후 이 상태가 가능해지더라도 migration을 바로 실행하면 안 됩니다. 운영 백업, 복구 가능성, 적용 담당자 승인, 실행 시간, 모니터링 및 중단 계획을 별도로 확인해야 합니다.
...
version `20260630090000` 행의 실제 기록 여부는 의도적으로 `WARN`으로 남습니다. 이 때문에 현재 자동 판정의 정상 최선 결과는 `REVIEW_REQUIRED`이며, `PREFLIGHT_PASS`는 향후 자동 확인을 위한 예약 상태입니다. 권한 있는 사람이 Supabase의 migration history 화면 또는 별도 승인된 읽기 절차로 중복 적용 여부를 확인해야 합니다. 사람이 migration version을 별도로 확인하기 전에는 운영 migration을 실행하면 안 됩니다. 이 runbook은 추가 SQL이나 우회 쿼리를 제공하지 않습니다.
...
이 항목이 완료되지 않으면 향후 `PREFLIGHT_PASS`가 가능해지더라도 Phase 1 migration을 실행하지 마세요.

/**
 * ⚠️ 미연결(unwired) 모듈 — 현재 사용자 앱의 어떤 화면도 이 엔진을 호출하지 않는다.
 *
 * 실제 런타임 판정 경로는 feedAnalysis.ts / ingredientQuality.ts / scoringPipeline.ts /
 * score.ts / displayVerdict.ts 다. 이 파일은 Phase 2 canonical 규칙(analysis_rules) 이행 때
 * 재사용할 규칙 평가 시맨틱과 그 회귀 테스트(ruleEngine.test.ts)를 보존하기 위해 남겨둔다.
 * 삭제하지 말고, 배선하려면 반드시 점수 회귀 테스트를 먼저 통과시킬 것.
 *
 * ── 원 설계 ────────────────────────────────────────────────────────────────

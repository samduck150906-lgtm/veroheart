...
const MACRO_META: { key: 'protein' | 'fat' | 'carb' | 'fiber' | 'moisture' | 'ash'; label: string; color: string }[] = [
  ...
  { key: 'ash', label: '조회분', color: '#94A3B8' },
...
function ChecklistRow({ ok, label, value, neutral }: { ok: boolean; label: string; value: string; neutral?: boolean }) {
  const color = neutral ? '#64748B' : ok ? '#16A34A' : '#DC2626';
  const bg = neutral ? '#F1F5F9' : ok ? '#DCFCE7' : '#FEE2E2';
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 0', borderTop: '1px solid #F1F5F9' }}>
      ...
      <span style={{ flex: 1, fontSize: 13.5, fontWeight: 600, color: '#334155' }}>{label}</span>
...
export default function FeedAnalysisCard({ product, profile }: { product: Product; profile: UserPetProfile }) {
  ...
  return (
    <section
      aria-label="사료성분 분석"
      style={{ background: '#fff', borderRadius: 24, padding: 20, marginBottom: 32, boxShadow: '0 8px 24px rgba(15,23,42,.06)', border: '1px solid rgba(15,23,42,.05)' }}
    >
      {/* 헤더 */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 16 }}>
        ...
        <span style={{ fontSize: 11, fontWeight: 800, color: '#475569', background: '#F1F5F9', borderRadius: 999, padding: '4px 10px', whiteSpace: 'nowrap' }}>
          규칙 기반 · AI 미사용
        </span>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '4px 2px 18px' }}>
        <div style={{ flexShrink: 0, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: '#94A3B8', marginTop: 2 }}>/ 100</div>
        </div>
        <div style={{ minWidth: 0 }}>
          ...
          <p style={{ margin: 0, fontSize: 13.5, fontWeight: 600, color: '#475569', lineHeight: 1.5 }}>{a.summary}</p>
        </div>
      </div>

      {/* 영양 구성 (보장성분 실측치 있을 때) */}
      {a.macros ? (
        <div style={{ background: '#F8FAFC', borderRadius: 16, padding: 16, marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 13.5, fontWeight: 800, color: '#0F172A' }}>영양 구성 (라벨 보장성분)</span>
            {a.calories && (
            ...
            {MACRO_META.map((m) => {
              ...
              return (
                <div key={m.key} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <span style={{ width: 92, flexShrink: 0, fontSize: 12.5, fontWeight: 700, color: '#475569' }}>{m.label}</span>
                  <span style={{ flex: 1, height: 8, borderRadius: 999, background: '#E5E8EB', overflow: 'hidden' }}>
                    <span style={{ display: 'block', width: `${Math.min(100, v)}%`, height: '100%', background: m.color, borderRadius: 999 }} />
                  </span>
                  <span style={{ width: 42, textAlign: 'right', fontSize: 12.5, fontWeight: 800, color: '#0F172A', fontVariantNumeric: 'tabular-nums' }}>{Math.round(v)}%</span>
                </div>
              );
            })}
          </div>
          {a.macrosDMB && (
            <p style={{ margin: '12px 0 0', fontSize: 11.5, fontWeight: 600, color: '#64748B', lineHeight: 1.5 }}>
              건물기준(수분 제외) 조단백질 <b style={{ color: '#0F172A' }}>{a.macrosDMB.protein}%</b> · 조지방 <b style={{ color: '#0F172A' }}>{a.macrosDMB.fat}%</b>
              {a.calories && <> · 열량 배분 단백 {a.calories.distribution.protein}% / 지방 {a.calories.distribution.fat}% / 탄수 {a.calories.distribution.carb}%</>}
            </p>
          )}
        </div>
      ) : (
        <div style={{ background: '#F8FAFC', borderRadius: 16, padding: '14px 16px', marginBottom: 14, fontSize: 12.5, fontWeight: 600, color: '#64748B', lineHeight: 1.5 }}>
      ...
      <div style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 13.5, fontWeight: 800, color: '#0F172A', marginBottom: 2 }}>원료 품질 체크</div>
      ...
      <p style={{ margin: '6px 2px 0', fontSize: 11, fontWeight: 600, color: '#94A3B8', lineHeight: 1.5 }}>{a.disclaimer}</p>
    </section>
  );
}

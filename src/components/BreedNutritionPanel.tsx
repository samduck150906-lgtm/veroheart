...
const CONDITION_META: Record<string, { emoji: string; label: string; accentColor: string; bgColor: string }> = {
  ...
  '심장': { emoji: '❤️', label: '심장', accentColor: '#EF4444', bgColor: '#FFF1F2' },
...
function conditionStatusBadge(status: ConditionStatus) {
  const map: Record<ConditionStatus, { label: string; bg: string; color: string }> = {
    ...
    unknown:    { label: '확인 불가', bg: '#F3F4F6', color: '#4B5563' },
...
function RuleRow({ rule }: { rule: BreedRuleResult }) {
  ...
  const confidenceColor = rule.confidence === 'high' ? '#6366F1' : rule.confidence === 'medium' ? '#F59E0B' : '#9CA3AF';
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: '12px' }}>
        <div style={{ fontSize: '13px', fontWeight: 800, color: rule.status === 'pass' ? '#16A34A' : rule.status === 'fail' ? '#DC2626' : '#6B7280' }}>
          {rule.actualDisplay}
        </div>
...
function ConditionCard({ result }: { result: BreedConditionResult }) {
  ...
  const meta = CONDITION_META[result.condition] ?? { emoji: '•', label: result.condition, accentColor: '#6B7280', bgColor: '#F9FAFB' };
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ textAlign: 'left' }}>
            ...
            <div style={{ fontSize: '11.5px', color: '#6B7280', fontWeight: 500, marginTop: '1px' }}>{result.note}</div>
          </div>
        </div>
      ...
      {open && (
        <div style={{ padding: '12px 16px', background: '#FFFFFF' }}>
          {result.status === 'bonus_only' ? (
            <div>
              <p style={{ fontSize: '12.5px', color: '#6B7280', fontWeight: 500, marginBottom: '8px', lineHeight: 1.6 }}>
              ...
              {result.skinBonusIngredients && result.skinBonusIngredients.length > 0 ? (
                ...
                <p style={{ fontSize: '12px', color: '#9CA3AF', fontWeight: 500 }}>보조 성분 미확인</p>
              )}
            </div>
            ...
            <div>
              ...
              {result.status === 'unknown' && (
                <p style={{ fontSize: '11.5px', color: '#9CA3AF', fontWeight: 500, marginTop: '10px', lineHeight: 1.6, padding: '8px', background: '#F9FAFB', borderRadius: '8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
          ...
          <div
            style={{
              ...
              background: netPositive ? '#22C55E' : netNeutral ? '#6B7280' : '#EF4444',
              color: '#fff',
            }}
          >
          ...
          <div>
            <div style={{ fontSize: '12.5px', fontWeight: 800, color: '#1E40AF', marginBottom: '4px' }}>
              ...
              <span style={{
                ...
                background: result.activityLevel === 'high' ? '#DCFCE7' : result.activityLevel === 'low' ? '#FEE2E2' : '#F3F4F6',
